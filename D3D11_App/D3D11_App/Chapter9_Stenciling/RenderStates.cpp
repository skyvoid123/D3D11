...
// We are not rendering backfacing polygons, so the BackFace settings do not matter.
	...
	// DrawReflectionDSS
	//
	D3D11_DEPTH_STENCIL_DESC drwaReflectionDesc;
	drwaReflectionDesc.DepthEnable = TRUE;
	drwaReflectionDesc.DepthWriteMask = D3D11_DEPTH_WRITE_MASK_ALL;
	drwaReflectionDesc.DepthFunc = D3D11_COMPARISON_LESS;
	drwaReflectionDesc.StencilEnable = TRUE;
	drwaReflectionDesc.StencilReadMask = 0xff;
	drwaReflectionDesc.StencilWriteMask = 0xff;

	drwaReflectionDesc.FrontFace.StencilFailOp = D3D11_STENCIL_OP_KEEP;
	drwaReflectionDesc.FrontFace.StencilDepthFailOp = D3D11_STENCIL_OP_KEEP;
	drwaReflectionDesc.FrontFace.StencilPassOp = D3D11_STENCIL_OP_KEEP;
	drwaReflectionDesc.FrontFace.StencilFunc = D3D11_COMPARISON_EQUAL;
	// We are not rendering backfacing polygons, so the BackFace settings do not matter.
	drwaReflectionDesc.BackFace.StencilFailOp = D3D11_STENCIL_OP_KEEP;
	drwaReflectionDesc.BackFace.StencilDepthFailOp = D3D11_STENCIL_OP_KEEP;
	drwaReflectionDesc.BackFace.StencilPassOp = D3D11_STENCIL_OP_KEEP;
	drwaReflectionDesc.BackFace.StencilFunc = D3D11_COMPARISON_EQUAL;
	HR(device->CreateDepthStencilState(&drwaReflectionDesc, &DrawReflectionDSS));
