...
VertexOut VS(VertexIn vin)
{
    ...
    vout.PosW.y = gHeightMap.SampleLevel(samLinear, vin.Tex, 0).r;
...
bool AabbOutsideFrustumTest(float3 center, float3 extents, float4 planes[6])
{
    for (int i = 0; i < 6; ++i)
    {
        // If the box is completely behind any of the frustum planes
		// then it is outside the frustum.
        if (AabbBehindPlaneTest(center, extents, planes[i]))
        {
            return true;
        }
    }
...
PatchTess PatchHS(InputPatch<VertexOut, 4> patch)
{
    PatchTess pt;
    ...
    float3 boxCenter = (vMin + vMax) / 2;
    float3 boxExtents = (vMax - vMin) / 2;
    ...
    else
    {
        ...
        float3 e0 = (patch[0].PosW + patch[2].PosW) / 2;    // Left
        float3 e1 = (patch[0].PosW + patch[1].PosW) / 2;    // Top
        float3 e2 = (patch[1].PosW + patch[3].PosW) / 2;    // Right
        float3 e3 = (patch[2].PosW + patch[3].PosW) / 2;    // Bottom
        float c = (patch[0].PosW + patch[1].PosW + patch[2].PosW + patch[3].PosW) / 4;  // Center
            ...
            const OutputPatch<HullOut, 4> quad)
{
    ...
    dout.PosW.y = gHeightMap.SampleLevel(samLinear, dout.Tex, 0).r;
            ...
            uniform bool gFogEnabled) : SV_Target
{
    ...
    float leftY = gHeightMap.SampleLevel(samLinear, leftTex, 0).r;
    float rightY = gHeightMap.SampleLevel(samLinear, rightTex, 0).r;
    float bottomY = gHeightMap.SampleLevel(samLinear, bottomTex, 0).r;
    float topY = gHeightMap.SampleLevel(samLinear, topTex, 0).r;
    ...
    float4 c0 = gLayerMapArray.Sample(samLinear, float3(pin.TiledTex, 0));
    float4 c1 = gLayerMapArray.Sample(samLinear, float3(pin.TiledTex, 1));
    float4 c2 = gLayerMapArray.Sample(samLinear, float3(pin.TiledTex, 2));
    float4 c3 = gLayerMapArray.Sample(samLinear, float3(pin.TiledTex, 3));
    float4 c4 = gLayerMapArray.Sample(samLinear, float3(pin.TiledTex, 4));
